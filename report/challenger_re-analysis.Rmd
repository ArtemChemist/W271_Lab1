...
```{r}
model_zero <- glm(formula=failure~Temp, family=binomial(), data=oring_df)
summary(model_zero)
```
...
bs <- function(formula, data, indices) {
  ...
  return(c(coef(fit), summary(fit)$deviance))
} 
```

```{r Boot 1, warning=FALSE}
# bootstrapping with 1000 replications
results <- boot(data=oring_df, statistic=bs,
   R=1000, formula=failure~Temp)
```

```{r}
results$t <- results$t[results$t[,3] > 0.01,]
results$R <- nrow(results$t)
```

```{r}
plot(results, index=1) # intercept
plot(results, index=2) # Temp
```

```{r bootstrap graphing}
int.conf <- boot.ci(results, conf = 0.9, type="bca", index=1) # intercept
slope.conf <- boot.ci(results, conf = 0.9, type="bca", index=2) # Temp
```

``` {r}
temps <- seq(10,100)
int_var <-sum((results$t[,1]-results$t0[1])^2)/(results$R-1)
slope_var <-sum((results$t[,2]-results$t0[2])^2)/(results$R-1)

# For a given temperature, calculate predicted CI of logit.OR
# For this temp calculate OR
# Repear that for each temp
#odd.ratio.high
```

```{r Bootstrap w loop, warning=FALSE}
...
for (i in 1:1000){
  #run a model on a sample
  
  new_data = oring_df[sample(idx_seq, 23, replace = TRUE),]
  
  model_i <- glm(formula = failure ~ Temp, family = binomial(), data = new_data)
  if (model_i$deviance > 0.001) {
    counter = counter + 1
...
intercept_boot_var <- var(intercepts) # standard errors of intercept
intercept_boot_var

slope_boot <- mean(slopes)# bootstrap estimate of slope
slope_boot

slope_boot_var <- var(slopes) # standard errors of slope
slope_boot_var
...
for (i in 1:1000){ 
  theta_star[i] <- mean()
    ...
    intercepts[counter] <- rnorm(length(df),mean = 3617.7 ,sd = 464.6)
    slopes[counter] <- model_i$coefficients[2]

  }
```

```{r Inference Bootstrap}
...
logit_lower <- logit - 1.96*sqrt(intercept_boot_var + slope_boot_var*temps^2  )
...
infer_df = cbind.data.frame(temps, boot_pi, boot_pi_lower, boot_pi_upper)
infer_df
...
for (temp in 10:100){
  counter = counter+1
  Temp <- temp
  modeled_pi[counter] <- predict(model_zero, newdata = data.frame(Temp), type = "response")
}

infer_df = cbind(infer_df, modeled_pi)
```

```{r graph}

ggplot(data = infer_df) +
  aes(x = temps, y = boot_pi) +
  geom_line()
