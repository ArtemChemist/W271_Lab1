...
library(tidymodels)
oring_boot <- bootstraps(oring_df,
  times = 1e3,
  apparent = TRUE
)
```
...
```{r Boot 1}
oring_models <- oring_boot %>%
  mutate(
      model = map(splits, ~ glm(formula = failure ~ Temp, family = binomial(), data = .)
    ),
...
```{r}
#oring_models_filtered <- filter(oring_models, term == "Temp")
oring_coefs <- oring_models %>%
  unnest(coef_inf)
```

```{r bootstrap graphing}
oring_coefs %>% filter(term == 'Temp') %>%
  ggplot(aes(estimate))+
