...
> For cereals with normalized sugar content up to approximately average, there is roughly eqaul chance of finding them on 4'th and 3'd shelfs. Assuming the first shelf is at teh bottom and the 4'th is at the top,  an average health-concious adult might see them there. As the normalized sugar content approaches the higher end of the spectrum, the chances of finding this cereal on the second shelf, where a kid might see it, is growing dramatically.
...
neg_plot <- ggplot(
  ...
  guides(
    fill = guide_legend(title = "Day", keywidth = 0.5, keyheight = 0.5,
                        direction = "horizontal"),
    colour = guide_legend(title = "Day", keywidth = 0.5, keyheight = 0.5,
                          direction = "horizontal"),
  ) + theme(axis.title.y = element_blank(), axis.title.x = element_blank()) + 
  xlim(0, 2)
...
ggarrange(NULL, NULL, neg_plot, pos_plot, NULL, NULL, tot_plot, sest_plot,
  labels = c("", "", 
             "Negative romance", "Positive romance",
             "", "", "Total drinks", "Self-esteem"),
...
```{r Poisson}
model_neg_rel_saturday <- glm(formula = as.integer(numall) ~ nrel,
                              family = poisson,
                              data = filter(drinks, dayweek == "6"))
summary(model_neg_rel_saturday)

model_neg_rel_tuesday <- glm(formula = as.integer(numall) ~ nrel,
                             family = poisson,
...
model_des_tuesday <- lm(formula = as.numeric(desired) ~ nrel,
                        data = filter(drinks, dayweek == "2"))
summary(model_des_tuesday)

model_des_saturday <- lm(formula = as.numeric(desired) ~ nrel,
                         data = filter(drinks, dayweek == "6"))
...
```{r Self-Esteem}
model_neg_rel_saturday <- glm(formula = as.integer(numall) ~ nrel + rosn
                              + rosn:nrel, family = poisson,
                              data = filter(drinks, dayweek == "6"))

summary(model_neg_rel_saturday)

model_neg_rel_tuesday <- glm(formula = as.integer(numall) ~ nrel + rosn
                             + rosn:nrel, family = poisson,
                             data = filter(drinks, dayweek == "2"))
summary(model_neg_rel_tuesday)
```