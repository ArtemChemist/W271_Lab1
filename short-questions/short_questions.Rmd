...
cereal %>% 
  ...
  ggplot(data = ., aes(x = indicators, y = values)) +
    geom_boxplot() + geom_jitter(width = 0.125) + theme_minimal()
...
library(nnet)
model_cereal_shelves_linear <- multinom(formula = Shelf ~ fat_g_norm +
                                                          sugar_g_norm +
                                                          sodium_mg_norm,
                                                          data = cereal)


model_cereal_shelves_quadratic  <- multinom(formula = Shelf ~ fat_g_norm +
                                                          sugar_g_norm +
                                                          sodium_mg_norm +
