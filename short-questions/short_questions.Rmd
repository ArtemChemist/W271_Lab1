...
MASS::parcoord(cereal[c("Shelf", "fat_g_norm", "sodium_mg_norm", "sugar_g_norm")], col = colors()[12 * cereal$Shelf])
```
...
model_cereal_shelves_trim <- multinom(formula = Shelf ~ sugar_g + sodium_mg, data = cereal)
...
```{r create figure 3.3}
fat_mean <- mean(cereal$fat_g)
na_mean <- mean(cereal$sodium_mg)

df_to_plot <- data.frame(fat_g = rep(fat_mean, times = 100), sodium_mg = rep(na_mean, times = 100), sugar_g = seq(1, 100) / 100)
...
shelf_vs_sugar_plot <- df_to_plot %>%
  pivot_longer(cols = c(4:7), names_to = "indicators", values_to = "shelf values") %>%
  ggplot(data = ., aes(x = sugar_g, y = values, colour = indicators)) +
  geom_line() +
  theme_minimal()
