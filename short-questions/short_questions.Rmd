...
cereal3 <- data.frame(
  sugar = stand02(12/28, 0, 20),
  fat = stand02(0.5/28, 0, 5),
  sodium = stand02(130/28, 0, 330)
)
cereal3
...
```{r create figure 3.3}
sugar <- stand02(seq(0, 20, by=0.1), 0, 20)
mean_fat <- replicate(201, mean(stand01(cereal2$fat)))
mean_sodium <- replicate(201, mean(stand01(cereal2$sodium)))
cereal4 <- data.frame(
  sugar = sugar,
  fat = mean_fat,
  sodium = mean_sodium
...
plot.data <- data.frame(prediction = pi.hat.2, observed = seq(0, 20, by=0.1))
...
se.2 <- summary(model_cereal_shelves_linear)$standard.errors[1,]
ci.2 <- data.frame(estimate = exp(coefs.2), lower = exp(coefs.2 - 1.96*se.2), upper =  exp(coefs.2 + 1.96*se.2))
