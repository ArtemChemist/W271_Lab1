...
```{r Self-Esteem}
model_neg_rel_saturday <- glm(formula = as.numeric(numall) ~ as.numeric(nrel) + , family = poisson, data = filter(drinks, dayweek =='6'))
summary(model_neg_rel_saturday )

model_neg_rel_tuesday <- glm(formula = as.numeric(numall) ~ as.numeric(nrel), family = poisson, data = filter(drinks, dayweek =='2'))
summary(model_neg_rel_tuesday )
```